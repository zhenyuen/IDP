...
unsigned long ls_b_timer;
unsigned long ls_fm_timer;
unsigned long ls_fl_timer;
unsigned long ls_fr_timer;
unsigned long off_line_timer;
int ls_timer_trigger = TIME_LS_TRIGGER;
...
void setup() {
  ...
  if (DEBUG_MODE) {
    debug_task_counter = HEAD_TO_DROPOFF_POINT;
...
void loop() {
  ...
  // --------------------------------------------------------------------------------
    ...
    if (step_counter == 0) {
      ...
      Serial.println("Step 1");
      if (dummy_type == DUMMY_MODULATED) {
        Serial.println("Heading to white box.");
        follow_line(0.6, DEF_SPEED);
        if (junction_reached()) {
          next_task(DROP_DUMMY);
        }
      } else {
        Serial.println("Heading to blue/red box.");
        if (acc.get_state() == ASCEND_STATE) {
          follow_line(0.9, DEF_SPEED);
        } else {
          follow_line(0.6, DEF_SPEED);
...
void rotate_90(bool cw) {
  drive_system.rotate(DEF_SPEED, cw);
  delay(2000); // Over-rotate, correct with realign in opposite direction.
  drive_system.brake();
  return;
}
...
void exit_rbbox(bool repeat) {
  ...
  rotate_90(repeat);
  drive_system.steer(DEF_SPEED, repeat, 0.6);
  delay(500); // Make sure back line sensor crosses middle line
  realign(repeat, false);
}
...
void toggle_ls_b() {
  if (millis() - ls_b_timer < ls_timer_trigger) return;
  ls_b_timer = millis();
  ls_b = !digitalRead(LS_B_PIN);
}

void toggle_ls_fm() {
  if (millis() - ls_fm_timer < ls_timer_trigger) return;
  ls_fm_timer = millis();
  ls_fm = !digitalRead(LS_FM_PIN);
}

void toggle_ls_fl() {
  if (millis() - ls_fl_timer < ls_timer_trigger) return;
  ls_fl_timer = millis();
  ls_fl = !digitalRead(LS_FL_PIN);
}

void toggle_ls_fr() {
  if (millis() - ls_fr_timer < ls_timer_trigger) return;
  ls_fr_timer = millis();
  ls_fr = !digitalRead(LS_FR_PIN);
}
